 ...
 *
 * Usage:
 *   npx tsx src/scripts/hooks/bootstrap-readmes.ts              # All
 *   npx tsx src/scripts/hooks/bootstrap-readmes.ts original     # French only
 *   npx tsx src/scripts/hooks/bootstrap-readmes.ts cz           # Czech only
 *   npx tsx src/scripts/hooks/bootstrap-readmes.ts cz 001       # Specific carnet
 */
...
<!-- SYNC: content/_original/${info.carnet}/README.md -->
<!-- WORKER: @${user} since ${timestamp.split('T')[0]} -->
...
<!-- BEGIN:SYNC:ORIGINAL -->
<!-- Items synced from content/_original/${info.carnet}/README.md appear here -->
<!-- END:SYNC:ORIGINAL -->
...
async function bootstrapLanguage(language: string, filterCarnet?: string): Promise<number> {
  ...
  for (const carnet of carnets) {
    ...
    // Generate README
    const content = language === '_original'
...
async function main(): Promise<void> {
  ...
  if (filterLang) {
    ...
    languages = readdirSync(contentPath)
