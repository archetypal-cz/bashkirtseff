...
Prebuilt data also lives in `public/data/` — do not hand-edit.

- `filter-index.json` is built by `src/lib/filter-index-builder.ts`, run from an
  Astro integration in `astro.config.mjs` at `astro:config:setup`, so **every**
  `astro dev` / `astro build` regenerates it from `content/`. It has to live
  inside the frontend: the production image (`Dockerfile`) only copies
  `src/shared`, `src/frontend` and `content/` and only runs `astro build`, so a
  `src/scripts/` generator never ran on deploy and the shipped index silently
  went stale (new glossary tags → "Show in diary" filtered to 0 entries).
  `src/scripts/build-filter-index.ts` (`just fe-filter-index`) is a thin CLI
  wrapper around the same module. The write is skipped when only the `built`
  timestamp would change, so rebuilds don't produce git noise.
- `offline-manifest.json` is built the same way, by `src/lib/offline-manifest-builder.ts`
  from a sibling integration — but **only on `astro build`**, not `astro dev`:
  dev has no offline download to invalidate and rewriting the hash on every dev
  start would dirty a committed file. `stores/offline.ts` is its only consumer
  and reads only `commit` (stamped onto each download, then compared in
  `checkFreshness()` to flag stale downloads — `built`/`version` are
  informational), so the manifest is rewritten only when `commit` or `version`
  changes. Every deploy is a new commit, so per-deploy freshness holds while
  same-commit rebuilds stay diff-free. The commit comes from `git rev-parse`
  (walking up from `src/frontend`, which reaches `/app/.git` in the image — the
  Dockerfile already copies `.git` and installs git), overridable via
  `OFFLINE_MANIFEST_COMMIT` / `GIT_COMMIT` / `GITHUB_SHA`. If no commit resolves,
  the existing file is kept rather than stamped with a wrong hash. The committed
  copy is only a fallback seed; the shipped one comes from the deploy build.
  `src/scripts/build-offline-manifest.ts` (`just fe-offline-manifest`) is a thin
  CLI wrapper around the same module.
