...
This is an AstroJS Progressive Web App for reading Marie Bashkirtseff's diary. The content comes from the parent project (`../src/`). This frontend focuses on:
...
The diary content lives in `../src/`. Configure collections:
...
| Content Path (URLs) | `cz` | `fr` | `en` | `_original` |
...
1. Check symlinks to `../src/` are correct
