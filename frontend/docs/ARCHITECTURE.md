...
### 7. Hosting: Docker on aretea.cz

**Decision**: Self-host on existing server as a separate container.

**Note**: The current `bashkirtseff-simple` container (static nginx) remains at `b.aretea.cz`. The new Astro PWA will run as a separate container with its own NPM endpoint (e.g., `app.bashkirtseff.cz` or similar).
...
- Existing infrastructure on aretea.cz
