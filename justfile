...
help:
    ...
    @echo "  just verify-carnet cz 001 # Structural integrity gate for a carnet (lang: cz|uk|en|fr|es); footnote-glue runs as a WARN sidecar"
    @echo "  just verify-carnet-all cz # Run that gate across every carnet (+ check-comments at the end)"
    @echo "  just check-comments cz    # %% marker-shape gate for whole trees (splices, unclosed/multi-line blocks, glued spans); --selftest"
    @echo "  just fix-marker-shapes --tree cz --carnet 070 --apply  # Repair the %% shapes check-comments flags (dry run without --apply)"
