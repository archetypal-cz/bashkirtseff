 ...
 *
 * Usage:
 *   npx tsx scripts/hooks/init-source-hashes.ts [language] [carnet]
 *
 * Examples:
 *   npx tsx scripts/hooks/init-source-hashes.ts           # All languages, all carnets
 *   npx tsx scripts/hooks/init-source-hashes.ts cz        # Czech only
 *   npx tsx scripts/hooks/init-source-hashes.ts cz 001    # Czech carnet 001 only
 */
...
function getLanguages(filterLang?: string): string[] {
  ...
  const langs = readdirSync(srcPath)
    .filter(d => !d.startsWith('_') && !d.endsWith('.md'))
    .filter(d => {
      const langPath = join(srcPath, d);
...
function getCarnets(language: string, filterCarnet?: string): string[] {
  const root = getProjectRoot();
  const langPath = join(root, 'src', language);
...
function getEntries(language: string, carnet: string): string[] {
  const root = getProjectRoot();
  const carnetPath = join(root, 'src', language, carnet);
  ...
  for (const lang of languages) {
    ...
    for (const carnet of carnets) {
      ...
      for (const entry of entries) {
        const translationPath = join(root, 'src', lang, carnet, entry);
        const originalPath = join(root, 'src', '_original', carnet, entry);
