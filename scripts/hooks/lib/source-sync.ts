...
export function findTranslations(originalPath: string): string[] {
  ...
  const match = originalPath.match(/src\/_original\/(\d{3})\/(\d{4}-\d{2}-\d{2}\.md)$/);
  ...
  for (const lang of languages) {
    const translationPath = join(root, 'src', lang, carnet, filename);
    if (existsSync(translationPath)) {
...
export function checkSourceChanges(originalPath: string): {
  ...
  for (const translationPath of translations) {
    ...
    if (storedHash !== newHash) {
      result.changed = true;
      const langMatch = translationPath.match(/src\/([^/]+)\/\d{3}\//);
