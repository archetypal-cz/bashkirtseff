...
%% 2025-12-07T17:30:00 TR: REVIZE podle editora: 1) "romanesque" přeloženo jako "náchylná k romantickým představám" (= ovlivněná romány, ne jen "romantická"); 2) "un beau matin" → "jednoho dne náhle" (idiomatický náhly odchod); 3) "drôle de machine" → "podivný stroj" (ironický tón); 4) "par esprit de pose" → "ze samé afektovanosti" (přirozený český výraz místo doslovného "z ducha pózy"); 5) "gobeuse" → "naivní" (důvěřivá je příliš ušlechtilé); 6) "rukavičkovaná" opraveno na "jak nosíte rukavice" (gramaticky správně); 7) "J'en restai fière" → "Zůstala jsem na to hrdá" (zdůraznění přetrvávající pýchy); 8) "ne hezká" → "nehezká" (jedno slovo). %%
...
Cítila jsem, jak blednu před tímto projevem citlivosti. Ostatně byla jsem dost neduživá, hubená a nehezká, což nezabránilo tomu, aby mě všichni považovali za bytost, která se musí osudově, absolutně stát tím nejkrásnějším, nejzářivějším a nejvelkolepějším. Maminka šla k jednomu Židovi, který věštil budoucnost:
...
[^00.29.1]: Pozn. red.: *Romanesque* (fr.) - náchylná k romantickým představám pod vlivem románů, které četla. V 19. století bylo běžné, že vzdělané ženy z vyšších vrstev, které se ocitly v obtížné životní situaci (jako rozvod nebo ovdovění), hledaly zaměstnání jako guvernantky nebo vychovatelky v bohatých rodinách. Tato pozice jim umožňovala zachovat si určitý společenský status.
