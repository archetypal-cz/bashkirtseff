---
id: RUSSIAN
name: Russian
aliases:
  - Russian
type: Culture
category: culture/languages
research_status: Basic
last_updated: 2026-03-06
---
# Russian

**Research Status**: Intermediate
**Last Updated**: 2026-02-12
**Type**: Language

## Overview

Russian is Marie's native family language. While she writes her diary in French, she occasionally uses Russian words, especially for family terms, emotional expressions, or when quoting family members. Russian code-switching signals intimacy, family identity, and connection to her Ukrainian-Russian roots.

## Common Russian Words in Marie's Diary

- "Moussia" - Marie's family nickname (diminutive of Maria)
- "diadia" - uncle
- "Michka" - diminutive (bear)
- "Vassya" - diminutive of Vassili
- "stariovoi" - old man / watchman
- "Iconostase" - iconostasis (Orthodox church screen)
- "Lavra" - lavra (Orthodox monastery)
- Patronymics for Russian relatives and acquaintances
- Terms for Russian Orthodox religious practices

## Usage Notes

- When Marie writes in Russian within a paragraph, tag it with #Russian
- If the paragraph also contains French (usual case), also tag with #French
- If a paragraph is entirely in Russian, only tag with #Russian (no #French)

## Common Patterns

- Family terms (diminutives, endearments)
- Quotations from family members
- Expressions of strong emotion
- References to Russian customs
- Orthodox religious terminology

## Usage Notes for Translators

Russian code-switching often signals:
- Intimacy and family context
- Connection to Russian identity
- Childhood/domestic sphere
- Religious/cultural traditions

## Related Languages

- [[FRENCH]] - primary language (diary written in French)
- [[ENGLISH]] - another language she uses
