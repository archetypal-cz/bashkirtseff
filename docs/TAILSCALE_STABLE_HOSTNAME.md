...
4. **DNS unpredictability**: This makes the MagicDNS name unpredictable, though in this workflow it appears SSH is connecting to a fixed hostname `aretea` (the deployment target), so this may not be causing issues currently.
